...
namespace cmt {
                             ...
                             const Mat desc_bg, const Point2f center)
    {
        ...
        {
              ALOG("Matcher::initialize err target_length:%d database:%d\n",target_length,database.cols);
        }
        ...
        if (new_kpt_des.size > 0 && learnDatabase.rows > firstKeypontsCount)
        {
            ...
            for (size_t i = 0; i < matches.size(); i++)
            {
                ...
                else
                {
                    ...
                    {
                        ALOG("EVENT updateModel: ERR 新加入的特征点匹配到了原始特征 mIndex < firstKeypontsCount\n");
                    }
                }
