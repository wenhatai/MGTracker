...
//        setIdentity(KF.errorCovPost, Scalar::all(.1));
    ...
    Rect Predictor::calcPredictRect(cv::Size preRectSize,cv::Size img_size)
    {
        ...
        if (TIMEDEBUG) {
            ALOG("CMTTIME  Predictor::calcPredictRect:%.3f\n",(now_ms()-startCTime_compute)*1000.0/CLOCKS_PER_SEC);
        }
