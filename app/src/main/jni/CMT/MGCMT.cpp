...
//        //MGX_PERF_LOG_BEGIN_TIME(calcInitKeypointsAndOptimumPerfParam)
        clock_t startCTime_calcInitKeypointsAndOptimumPerfParam= clock();
        ...
        if (TIMEDEBUG)
        {
            printf("CMTTIME calcInitKeypointsAndOptimumPerfParam:%.3f,scale:%.3f,th:%d\n",(clock()-startCTime_calcInitKeypointsAndOptimumPerfParam)*1000.0/CLOCKS_PER_SEC,_perfAdapter._initScale,_perfAdapter._initFastThreshold);
...
//        _pLearn_MGT_T2->initialize(grayImage,keypoints,targetRect,_perfAdapter);
    ...
    void CMT::processFrame(Mat &cur_orig_im_RGBA)
    {
        ...
        float preFrameTime = (clock()- startCTime_all)/(float)CLOCKS_PER_SEC;
        ...
        if (TIMEDEBUG) {
            printf("EVENT processFrame matchPrecent:%.3f frameRate:%.2f\n",_matchedPrecent,frameRate);
        }
...
//            }
    ...
    void CMT::processFrame_inner(Mat curRGBImage)
    {
        ...
        clock_t startCTime_calcRuningKeypoints= clock();
...
//            //MGX_PERF_LOG_APP("detect", "检测特征点", detect, false);
            printf("CMTTIME detect:%.3f keypoints.size:%lu\n",(clock()- startCTime_calcRuningKeypoints)*1000.0/CLOCKS_PER_SEC,keypoints.size());
