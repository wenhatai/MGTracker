...
//        detector = new cv::BRISK;
        ...
        #else
          detector = new cv::FastFeatureDetector(30);
          descriptorExtractor = new cv::BRISK;
        #endif
}

bool LibCMT::initialise(cv::Mat im_gray0, cv::Point2f topleft, cv::Point2f bottomright)
{
...
void LibCMT::estimate(const std::vector<std::pair<cv::KeyPoint, int> >& keypointsIN, cv::Point2f& center, float& scaleEstimate, float& medRot, std::vector<std::pair<cv::KeyPoint, int> >& keypoints)
{
...
double start_time;
double end_time;
//std::vector<cv::KeyPoint> threadKeyPoints[MULTI_THREAD_NUM];
std::vector<cv::KeyPoint> keypoints;

void addElementIfNeed(std::vector<cv::KeyPoint> keyPoints,cv::KeyPoint keyPoint){
    bool find = false;
    for(int i = 0;i< keyPoints.size();i++){
        if(keyPoints[i].pt == keyPoint.pt && keyPoints[i].size == keyPoint.size &&
...
void *detectThread(void *arg){
    ProcessFrameParam *param = (ProcessFrameParam *) arg;
    LibCMT *cmt = param->cmt;
    cv::Mat im_gray = param->im_gray;
    int processNum = param->processNum;
    cv::Mat process_gray;
    int startX,startY;
    startX = startY = 0;
    if(processNum < MULTI_THREAD_NUM/2){
        int width = im_gray.cols*2/MULTI_THREAD_NUM;
        int startX = im_gray.cols*processNum*2/MULTI_THREAD_NUM;
        process_gray = im_gray(cv::Rect(startX,0,width,im_gray.rows));
    }else{
        int height = im_gray.rows*2/MULTI_THREAD_NUM;
        int startY = im_gray.rows*(processNum-MULTI_THREAD_NUM/2)*2/MULTI_THREAD_NUM;
        process_gray = im_gray(cv::Rect(0,startY,im_gray.cols,height));
    }

//    int width = im_gray.cols/MULTI_THREAD_NUM;
//    int startX = im_gray.cols*processNum/MULTI_THREAD_NUM;
//    process_gray = im_gray(cv::Rect(startX,0,width,im_gray.rows));

    std::vector<cv::KeyPoint> processKeyPoint;
    cmt->detector->detect(process_gray, processKeyPoint);
    if(processNum < MULTI_THREAD_NUM/2){
        for(int i = 0;i< processKeyPoint.size();i++){
                processKeyPoint[i].pt.x = processKeyPoint[i].pt.x + startX;
                processKeyPoint[i].pt.y = processKeyPoint[i].pt.y + startY;
                addElementIfNeed(keypoints,processKeyPoint[i]);

        }
    }

//    keypoints.insert(keypoints.end(), processKeyPoint.begin(), processKeyPoint.end());
}

void LibCMT::processFrame(cv::Mat im_gray)
{
        start_time = now_ms();
                ...
                end_time = now_ms();
        //            ALOG("track time:%f",(end_time-start_time));
                ...
                end_time = now_ms();
//                ALOG("estimate time:%f",(end_time-start_time));
...
//                pthread_t tld[MULTI_THREAD_NUM];
//                ProcessFrameParam param[MULTI_THREAD_NUM];
//                for(int i = 0;i< MULTI_THREAD_NUM;i++){
//                    param[i].processNum = i;
//                    param[i].cmt = this;
//                    param[i].im_gray = im_gray;
//                    pthread_create(&tld[i],NULL,detectThread,&param[i]);
//                }
//
//                for(int i = 0;i< MULTI_THREAD_NUM;i++){
//                    pthread_join(tld[i],NULL);
//                }

                end_time = now_ms();
//                ALOG("detect time:%f",(end_time-start_time));
        ...
        end_time = now_ms();
//        ALOG("compute time:%f",(end_time-start_time));
        ...
        end_time = now_ms();
//        ALOG("knnMatch time:%f",(end_time-start_time));
        ...
        end_time = now_ms();
//             ALOG("end time:%f",(end_time-start_time));
}
...
bool LibCMT::Save(const char *path)
{
...
bool LibCMT::Load(const char *path)
{
        ALOG("En load");
