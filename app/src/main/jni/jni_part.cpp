...
JNIEXPORT jbyteArray JNICALL Java_com_melonteam_mgtracker_MGTracker_openTrack(JNIEnv *env, jobject, jbyteArray yuvData,jint dataType,jint x, jint y, jint width, jint height,jint imageWidth,jint imageHeight)
{
        ...
        #ifdef LIBCMT
        CMTinitiated = libCMT->initialize(getTrackMat(g_dataBuff,dataType,imageWidth,imageHeight),rect);
        #else
        CMTinitiated = g_cmt->initialise(getTrackMat(g_dataBuff,dataType,imageWidth,imageHeight),p1,p2);
        #endif

}

JNIEXPORT jintArray JNICALL Java_com_melonteam_mgtracker_MGTracker_getPredictTrack(JNIEnv *env,jobject,jint imageWidth,jint imageHeight){
  if (!CMTinitiated)
          return NULL;
